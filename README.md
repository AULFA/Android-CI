...
build the various AULFA applications.
...
|---------------------------|--------------|-----------|
|`one.lfa.no_signing`       |`true`/`false`|If `true`, no PGP signing of artifacts will occur|
|`one.lfa.directory.publish`|Any path      |If set, artifacts will be published to the named directory in Maven repository format|
...
|`AULFA_GITHUB_ACCESS_TOKEN`       |A GitHub access token used to access private NYPL repositories|
...
```
$ echo 'https://github.com/AULFA/example' > .ci-local/deploy-git-binary-source.conf
$ echo 'AULFA/android-binaries' > .ci-local/deploy-git-binary-target.conf
...
The above set of configuration files will cause binaries to be published to the `AULFA/android-binaries`
...
`https://github.com/AULFA/example`, showing the exact commit that
...
|File|Description|
|----|-----------|
|`firebase-apk.conf`|The name of the APK file to be deployed (such as `one.lfa.testing.app/build/outputs/apk/release/one.lfa.testing.app-release-unsigned.apk`)|
